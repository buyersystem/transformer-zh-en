...
class PositionalEncoding(nn.Module):
    ...
    sin/cos 的线性性质使 PE(pos+k) 可表示为 PE(pos) 的线性变换，
    模型因此能学习相对位置关系。
    """
    ...
    def forward(self, x):
        """x: [batch, seq_len, d_model], 返回加 PE + dropout 后的结果"""
...
def scaled_dot_product_attention(query, key, value, mask=None):
    ...
    mask: True 表示保留（可见），False 表示屏蔽（填 -inf）
    """
    d_k = query.size(-1)
...
class AddNorm(nn.Module):
    ...
    实现的是 Post-LN（论文原版顺序），与 Pre-LN 不同。
    """
...
class Transformer(nn.Module):
    ...
    三个入口：
    - forward(src, tgt):      训练，一次输出全部 logits
    - encode(src):            编码，返回 enc_output（推理时复用）
    - decode(tgt, enc, mask): 解码，支持逐步生成
    """
    ...
    def make_src_mask(self, src):
        """
        源序列 padding 掩码，返回 [batch, 1, 1, src_len]。
        在 encode 中转置得 [batch,1,src_len,src_len]，
        在 cross-attention 中广播到 [batch,1,tgt_len,src_len]。
        """
